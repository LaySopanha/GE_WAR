...
def evaluate(
    ...
    #Forward pass to get PROBABILITIES (not log-probabilities)
    ...
    #Vectorized rank computation over traces
    for t in trange(nb_traces_attacks, desc="Calculating GE Curve"):
        trace_indices = np.array([s[t] for s in shuffles])
        
        prob_slice = probs[trace_indices]    
        logp_slice = np.log(prob_slice + 1e-40) # This mimics the original's `np.log(prediction + 1e-40)` basically the same but faster for efficientcy 
